        ...
        switch (parser->state) {
            ...
            case STATE_PAYLOAD: {
                if (parser->payload_bytes_read + 1 > parser->current_len) {
                    logger_log(LOG_LEVEL_ERROR,
                               "payload read past frame length");

                    status = STREAMX_ERR_OVERFLOW;
                    goto reset_state;
                }

                /* Wire bytes are XOR'd with 0x55; store decoded byte */
                parser->payload_buf[parser->payload_bytes_read++] =
                    byte ^ 0x55;
...
streamx_status_t parser_tokenize_metadata(parser_t *parser,
    ...
    while (token != NULL) {
        ...
        *eq = '\0';
        ...
        parser->tokens[parser->token_count].key = key_copy;
        parser->tokens[parser->token_count].value = val_copy;

        if (strcmp(val, "ABORT") == 0) {
            free(key_copy);
            free(val_copy);

            parser->tokens[parser->token_count].key = NULL;
            parser->tokens[parser->token_count].value = NULL;
